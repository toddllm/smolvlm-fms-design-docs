...
- **Optimized inference and training**: FMS reimplements popular architectures (e.g., LLaMA, GPT‑BigCode) to leverage PyTorch-native features like `torch.compile`, scaled dot-product attention, and FSDP for efficient distributed training ([FMS github](https://github.com/foundation-model-stack/foundation-model-stac)).
...
- In the Transformers repo, each model has a configuration file, a modeling file, and optional processing modules. For example, SmolVLM includes:
  ...
  - `image_processing_smolvlm.py` for preprocessing images ([smolvlm github](https://github.com/huggingface/transformers/tree/main/src/transformers/models/smolvlm)).
